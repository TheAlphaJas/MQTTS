...
from trainer_v2 import Wav2TTS
...
# hop_length = 512
# n_mels = 80
# sample_rate = 22050  # or your audio sample rate

# # StyleEncoder settings
# style_dim = 128  # Output dimension of StyleEncoder
...
# --- Partial & safe checkpoint loading ---
if args.resume_checkpoint is not None:
    print(f"Loading checkpoint (safe mode): {args.resume_checkpoint}")
    ...
    print(f"✅ Loaded {len(compatible_state_dict)} tensors from checkpoint.")
    print(f"⚠️ Skipped keys (missing/mismatched): {skipped_keys}")
    print(f"⚠️ Missing keys in checkpoint: {missing}")
    print(f"⚠️ Unexpected keys in checkpoint: {unexpected}")
    
print("Total steps - ", 228908/args.batch_size)
# model = torch.load('./ckpt/last.ckpt')
# state_dict = torch.load('./ckpt/last.ckpt')['state_dict']
# model.load_state_dict(state_dict, strict=False)
# model = model.load_from_checkpoint("./ckpt/last.ckpt")
# model.load_checkpoint("./ckpt/last.ckpt")
wrapper.fit(model)
