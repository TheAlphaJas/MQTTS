...
class QuantizeDataset(data.Dataset):
    def __init__(self, hp, metapath):
        ...
        print (self.phoneset)
        if not hasattr(self.hp, 'speaker_embedding_dir') or self.hp.speaker_embedding_dir is None:
    ...
    def __getitem__(self, i):
        ...
        if hasattr(self.hp, 'speaker_embedding_dir') and self.hp.speaker_embedding_dir is not None:
            try:
                # Assuming embeddings are saved as {basename}.npy
                emb_path = os.path.join(self.hp.speaker_embedding_dir, os.path.splitext(_name)[0] + '.npy')
                if os.path.exists(emb_path):
                    speaker_embedding = np.load(emb_path).astype(np.float32)
                else:
                     print(f"Embedding not found for {_name} at {emb_path}")
            except Exception as e:
                print(f"Error loading embedding for {_name}: {e}")
        ...
        if speaker_embedding is None:
             ...
             try:
                 ...
                 print(f"Error extracting embedding for {dataname}: {e}")
